...
export default function Controls({ audioRef,
   ...
   currentSong,
    isPlaying, 
    setIsPlaying, setCurrentSong, isCard, setIsCard, setCurrentIndex }) {

  let currentIndex = songs.findIndex((song) => song.id === currentSong.id);

  // isCard ? playPause() : botaoStop()
  const playPause = () => {
    if (isPlaying) {
      setIsCard(!isCard)
  ...
  return (
    ...
    <div className='controlesMusica'>
      <div className='informacoesMusica'>
        <div className='musica'>{!isCard ? '' : currentSong.title}</div>
        <div className='autor'>{!isCard ? '' : currentSong.artist}</div>
      </div>
